    ...
    class Bicep < Rouge::RegexLexer
      ...
      def self.functions
        @functions ||= Set.new %w(
          ...
          dataUriToString dateTimeAdd dateTimeFromEpoch dateTimeToEpoch deployment empty endsWith 
          environment extensionResourceId filter first flatten format getSecret groupBy guid indexOf 
          int intersection items join json last lastIndexOf length listAccountSas listKeys listSecrets 
          loadFileAsBase64 loadJsonContent loadTextContent loadYamlContent managementGroup 
          managementGroupResourceId map mapValue max min newGuid objectKeys padLeft parseCidr pickZones 
          providers range readEnvironmentVariable reduce reference replace resourceGroup resourceId 
          shallowMerge skip sort split startsWith string subscription subscriptionResourceId substring take 
          tenant tenantResourceId toLower toObject toUpper trim union uniqueString uri uriComponent 
