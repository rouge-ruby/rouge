...
module Rouge
  module Lexers
    class DAVI < TemplateLexer
      title "DAVI"
      desc "The DaVinci scripting language (davinciscript.com)"
      tag 'davi'
      filenames '*.davi'
      mimetypes 'text/x-davi'

      option :start_inline, 'Whether to start with inline davi or require <?davi ... ?>. (default: best guess)'
      ...
      def initialize(*)
        ...
        # if truthy, the lexer starts highlighting with davi code
        # (no <?davi required)
      ...
      def self.detect?(text)
        return true if text.shebang?('davi')
        return false if /^<\?hh/ =~ text
        return true if /^<\?davi/ =~ text
      end
      ...
      def self.builtins
        Kernel::load File.join(Lexers::BASE_DIR, 'davi/keywords.rb')
        builtins
      end
      ...
      state :start do
        # We enter this state if we aren't sure whether the DAVI in the text is
        # delimited by <?davi (or <?=) tags or not. These two rules check
        ...
        rule(/[^$]+(?=<\?(davi|=))/i) { delegate parent; pop! }
      ...
      state :root do
        rule %r/<\?(davi|=)?/i, Comment::Preproc, :davi
