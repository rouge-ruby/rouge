...
module Rouge
  module Lexers
    class Isabelle < RegexLexer
      ...
      name = %r/[^\W\d][.\w']/
      num = %r/\d[\d_]*/
      num_hex = %r/0[xX][\da-fA-F][\da-fA-F_]*/
      num_oct = %r/0[oO][0-7][0-7_]*/
      num_bin = %r/0[bB][01][01_]*/
      ...
      state :root do
        ...
        rule %r/\w+/ do |m|
          if self.class.keyword_minor.include?(m[0])
            token Keyword::Pseudo
          elsif self.class.keyword_diag.include?(m[0])
            token Keyword::Type
          elsif self.class.keyword_thy.include?(m[0])
            token Keyword
          elsif self.class.keyword_theory_decl.include?(m[0])
            token Keyword
          elsif self.class.keyword_section.include?(m[0])
            token Generic::Heading
          elsif self.class.keyword_subsection.include?(m[0])
            token Generic::Subheading
          elsif self.class.keyword_theory_goal.include?(m[0])
            token Keyword::Namespace
          elsif self.class.keyword_theory_script.include?(m[0])
            token Keyword::Namespace
          elsif self.class.keyword_abandon_proof.include?(m[0])
            token Generic::Error
          elsif self.class.keyword_qed.include?(m[0])
            token Keyword
          elsif self.class.keyword_proof_goal.include?(m[0])
            token Keyword
          elsif self.class.keyword_proof_block.include?(m[0])
            token Keyword
          elsif self.class.keyword_proof_decl.include?(m[0])
            token Keyword
          elsif self.class.keyword_proof_chain.include?(m[0])
            token Keyword
          elsif self.class.keyword_proof_asm.include?(m[0])
            token Keyword
          elsif self.class.keyword_proof_asm_goal.include?(m[0])
            token Keyword
          elsif self.class.keyword_proof_script.include?(m[0])
            token Keyword::Pseudo
          elsif m[0] == '_'
            token Operator::Word
          elsif m[0] =~ name
        ...
        rule num, Name # display numbers as name
        rule num_hex, Num::Hex
