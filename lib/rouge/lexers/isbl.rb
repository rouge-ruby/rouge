...
module Rouge
  module Lexers
    class Isbl < RegexLexer
      title "ISBL"
      desc "The DIRECTUM programming language"
      tag 'isbl'
      filenames '*.isbl'
      ...
      job_block_properties_constants = %w(
      ...
      notice_block_properties_constants = %w(
      ...
      pseudoreference_code_constants = %w(
      ...
      requisite_ISBCertificateType_values_constants = %w(
      ...
      requisite_ISBEDocStorageType_values_constants = %w(
        STORAGE_TYPE_FILE
      ...
      requisite_compType2_values_constants = %w(
      ...
      # Rule identification
      rule_identification_constants = %w(
      ...
      script_block_properties_constants = %w(
      ...
      subtask_block_properties_constants = %w(
      ...
      system_reference_names_constants = %w(
      ...
      using_the_dialog_windows_constants = %w(
        ATTENTION_CAPTION
        cbsCommandLinks
        cbsDefault
      ...
      # Using the document
      using_the_document_constants = %w(
      ...
      using_the_EA_and_encryption_constants = %w(
      ...
      using_the_ISBL_editor_constants = %w(
        ISBL_SYNTAX
        NO_SYNTAX
        XML_SYNTAX
      )
      ...
      wait_block_properties_constants = %w(
      ...
      # Константы
      constants = sysres_constants + base_constants + 
      ...
      TCertificateInvalidationReason = %w(
      ...
      TLockableDevelopmentComponentType = %w(
        ldctStandardRoute
        ldctWizard
        ldctScript
        ldctFunction
      ...
      TMaxRecordCountRestrictionType = %w(
        mrcrtNone
        mrcrtUser
        mrcrtMaximal
        mrcrtCustom
      ...
      TStructuredObjectAttributeType = %w(
        soatString
        soatNumeric
        soatInteger
        soatDatetime
      ...
      # Name::Builtin : встроенные или библиотечные объекты (константы, перечисления)
      builtins = constants + enums
      
      # Определение идентификаторов
      id = /[a-zа-яё_!][a-zа-яё_0-9]*/i
      # Определение имен функций
      funcid = /\b[a-zа-яё_][a-zа-яё_0-9]*(?=[(])/i
      # Определение имен методов
      methid = /[a-zа-яё_0-9]*/i
      ...
      state :type do
        mixin :whitespace
        rule %r([ \t]*(?:#{interfaces.join('|')}))ix, Keyword::Type
      ...
      state :root do
				...
				rule /[:]/, Punctuation, :type
				rule /[.]/, Punctuation, :dotted
        rule /[\[\]();]/, Punctuation
        rule %r([&*+=<>/-]), Operator
        rule /(?<=[^\wа-яё]|^)(?:#{keywords.join('|')})(?=[^\wа-яё]|$)/ix, Keyword
        rule /(?<=[^\wа-яё]|^)(?:#{builtins.join('|')})(?=[^\wа-яё]|$)/ix, Name::Builtin
        rule /(?<=[^\wа-яё]|^)(?:#{predefined_variables.join('|')})(?=[^\wа-яё]|$)/ix, Name::Variable::Global
				rule funcid, Name::Function
