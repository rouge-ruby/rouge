...
module Rouge
  module Lexers
    class Stata < RegexLexer
      ...
      def self.KEYWORD_TYPES 
        @KEYWORD_TYPES ||= Set.new %w(byte int long float double str strL numeric string integer scalar matrix local global numlist varlist newlist)
      end
	  	  
      # Stata commands used with braces. Includes all valid abbreviations for 'forvalues'.
      def self.KEYWORDS_RESERVED 
        @KEYWORDS_RESERVED ||= Set.new %w(if else foreach forv forva forval forvalu forvalue forvalues to while in of continue break nobreak)
      end

      # Partial list of common programming and estimation commands, as of Stata 16
      # Note: not all abbreviations are included
      KEYWORDS = %w( do run include clear assert set mata log
      ...
      state :root do
        ...
        rule %r/\w+/ do |m|
          if self.class.KEYWORDS_RESERVED.include? m[0]
            token Keyword::Reserved	
          elsif self.class.KEYWORD_TYPES.include? m[0]
            token Keyword::Type
          else
            token Name
          end
